...
export default function StickerManager({ user, onLogout }) {
  ...
  const t = useRef(null);
  const save = useCallback((c,d) => { if(!user)return; setSaving(true); if(t.current)clearTimeout(t.current); t.current=setTimeout(async()=>{ try{await setDoc(doc(db,"collections",user.uid),{collected:c,duplicates:d,updatedAt:new Date().toISOString(),email:user.email});}catch(e){console.error(e);} setSaving(false); },500); },[user]);
